#include "testprocessmaxacceleration.h"
#include "utils/math_helper/math_helper.h"
#include "utils/reportHandle/reporthandle.h"

TestProcessMaxAcceleration::TestProcessMaxAcceleration(DEVICE::TYPE device, int axis, ControlParamType control, SpecificationParamType spec)
...
//            double maxVel = realAmp * 2 * PI * realFreq;
            ...
            if(maxAcc > mSpec.mSpecification[mAxis])
            {
            ...
            mReportInfo.testType = TEST_DEFINE::TEST_PROCESS::MAX_ACC;
            mReportInfo.specification = mSpec.mSpecification[mAxis];
            ReportHandle::getInstance()->createReportMaxAcceleration(mReportInfo,
