#include "testprocessmaxvelocity.h"
#include "utils/math_helper/math_helper.h"
#include "utils/reportHandle/reporthandle.h"

TestProcessMaxVelocity::TestProcessMaxVelocity(DEVICE::TYPE device, int axis, ControlParamType control, SpecificationParamType spec)
...
TestProcessMaxVelocity::~TestProcessMaxVelocity()
{

}

int TestProcessMaxVelocity::run()
{
    if(mPause == false)
    {
        switch(mStep)
...
//            double maxAcc = realAmp * (2*PI*realFreq) * (2*PI*realFreq);
            ...
            mReportInfo.testType = TEST_DEFINE::TEST_PROCESS::MAX_VEL;
            mReportInfo.specification = mSpec.mSpecification[mAxis];
            ReportHandle::getInstance()->createReportMaxVelocity(mReportInfo, mCtrlParam.mAmplitude[mAxis], mCtrlParam.mFrequency[mAxis], imgPath);
            break;
        }
        default: break;
        }
